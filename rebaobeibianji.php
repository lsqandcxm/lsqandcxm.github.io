...
//判断图片是否更新
if($sphoto){
	move_uploaded_file($sphoto,$photo);
	$sql="update fabu set photo='{$photo}' where id='{$id}'";
	$smt=$pdo->prepare($sql);
	$smt->execute();
...
$sql="update fabu set name='{$name}',fenlei='{$fenlei}',time='{$time}',price='{$price}',xinxi='{$xinxi}' where id='{$id}'";
$smt=$pdo->prepare($sql);
@$url=$fenlei.'.php';
if($smt->execute()){
